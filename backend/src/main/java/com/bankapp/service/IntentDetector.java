...
/**
 * Very lightweight keyword/regex-based intent detector. We use this instead of
 * LLM tool-calling because tinyllama / llama3.2:1b are not reliable
 * function-callers.
 *
 * Returns a structured "suggestedAction" the frontend can render as a confirm
 * card.
 */
public class IntentDetector {

    private static final Pattern AMOUNT = Pattern.compile(
            "(?:rs\\.?|₹|inr)?\\s*([0-9]+(?:\\.[0-9]+)?)\\s*(k|thousand|lakh|cr|crore)?", Pattern.CASE_INSENSITIVE);
    ...
    public static Map<String, Object> detect(String message) {
        ...
        if (mt.find()) {
            ...
            if (amt != null) {
                return action("TRANSFER", Map.of("amount", amt, "recipientUsername", mt.group(2)),
        ...
        if (md.find()) {
            ...
            if (amt != null) {
                return action("DEPOSIT", Map.of("amount", amt), "Deposit ₹" + amt + " into your account?");
            }
        }
        ...
        if (mw.find()) {
            ...
            if (amt != null) {
                return action("WITHDRAW", Map.of("amount", amt), "Withdraw ₹" + amt + " from your account?");
            }
        }
        return null;
    }
        ...
        try {
            ...
            if (unit != null) {
                ...
                if (unit.equals("k") || unit.equals("thousand"))
                    ...
                    value = value.multiply(BigDecimal.valueOf(100000));
                else if (unit.equals("cr") || unit.equals("crore"))
                    value = value.multiply(BigDecimal.valueOf(10000000));
            }
            return value;
