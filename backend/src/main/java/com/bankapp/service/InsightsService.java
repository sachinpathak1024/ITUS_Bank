...
public class InsightsService {
    ...
    public Map<String, Object> compute(Account account, int months) {
        ...
        categoryTotals.entrySet().stream().sorted((a, b) -> b.getValue().compareTo(a.getValue())).limit(8)
                .forEach(e -> {
                    Map<String, Object> row = new LinkedHashMap<>();
    ...
    public String csv(Account account, LocalDateTime start, LocalDateTime end) {
        ...
        for (Transaction t : all) {
            if (t.getCreatedAt() == null)
                continue;
            if (start != null && t.getCreatedAt().isBefore(start))
                continue;
            if (end != null && t.getCreatedAt().isAfter(end))
                continue;
            sb.append(t.getCreatedAt()).append(',').append(t.getTransactionType()).append(',')
                    .append(escape(t.getDescription())).append(',').append(t.getAmount()).append('\n');
        }
        return sb.toString();
    }
    ...
    public String html(Account account, LocalDateTime start, LocalDateTime end) {
        ...
        for (Transaction t : all) {
            ...
            count++;
            body.append("<tr><td>").append(t.getCreatedAt()).append("</td><td>").append(t.getTransactionType())
                    ...
                    .append(isCredit ? "+" : "-").append("₹").append(t.getAmount()).append("</td></tr>");
        }
        BigDecimal net = credits.subtract(debits);

        return "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>ITUS Bank Statement</title>" + "<style>"
                ...
                + "<h1>ITUS Bank — Account Statement</h1>" + "<div class='meta'>"
                + escapeHtml(account.getFullName() == null ? account.getUsername() : account.getFullName()) + " · A/C "
                + account.getAccountNumber() + "</div>" + "<div class='summary'>"
                + "<div><div class='label'>Transactions</div><div class='value'>" + count + "</div></div>"
                + "<div><div class='label'>Credits</div><div class='value' style='color:#059669'>+₹" + credits
                + "</div></div>" + "<div><div class='label'>Debits</div><div class='value' style='color:#dc2626'>-₹"
                + debits + "</div></div>" + "<div><div class='label'>Net</div><div class='value'>₹" + net
                + "</div></div>" + "</div>"
